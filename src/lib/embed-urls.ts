/**
 * Funções mais complexas (ordem aproximada): parseStreamUrl O(n) onde n = comprimento da URL/path.
 */

export type EmbedKind = "TWITCH_CHANNEL" | "TWITCH_VOD" | "YOUTUBE_VIDEO" | "YOUTUBE_LIVE_CHANNEL";
...
/**
 * O(n) — percorre segmentos do path e parâmetros da URL.
 */
export function parseStreamUrl(raw: string): ParsedEmbed {
  ...
  if (
    ...
    return parseYoutubeUrl(u);
  }

  return { kind: "INVALID", platform: "UNKNOWN", message: "Somente links do Twitch ou YouTube" };
}

/**
 * O(n) — segmentos do pathname.
 */
function parseTwitchPath(u: URL): ParsedEmbed {
 ...
 * O(n) — query e path.
 */
function parseYoutubeUrl(u: URL): ParsedEmbed {
  ...
  return {
    kind: "INVALID",
    platform: "UNKNOWN",
    message: "Use um vídeo do YouTube (watch?v=…), youtu.be/… ou canal /channel/UC… para live",
  };
}
...
export function buildEmbedSrc(
  ...
  switch (parsed.kind) {
    case "TWITCH_CHANNEL":
      return `https://player.twitch.tv/?channel=${encodeURIComponent(parsed.channel)}&parent=${encodeURIComponent(parent_hostname)}${muted_q || "&muted=false"}`;
    case "TWITCH_VOD":
      return `https://player.twitch.tv/?video=${encodeURIComponent(parsed.videoId)}&parent=${encodeURIComponent(parent_hostname)}${muted_q}`;
    case "YOUTUBE_VIDEO":
